...
def _render_header():
    ...
    st.markdown(
        "<div class='app-header'><span class='brand-title' style='font-weight:800; font-size:22px; color: var(--text)'>🏗️ Habisolute IA</span></div>",
        unsafe_allow_html=True,
    )
    st.caption(
        "Envie certificados em PDF e gere análises, gráficos, KPIs e relatório final com capa personalizada."
    )
...
def _auth_login_ui():
    ...
    with c2:
        pwd = st.text_input(
            "Senha", key="login_pass", type="password", label_visibility="collapsed", placeholder="Senha"
        )
    with c3:
        ...
        if st.button("Acessar", use_container_width=True):
            ...
            if not rec or not rec.get("active", True):
                ...
                log_event(
                    "login_fail", {"username": user, "reason": "not_found_or_inactive"}, level="WARN"
                )
    ...
    with st.expander("👤 Painel de Usuários (Admin)", expanded=False):
        ...
        with tab3:
            ...
            if df_log.empty:
                ...
                try:
                    _d = pd.to_datetime(
                        df_log["ts"].str.replace("Z", "", regex=False), errors="coerce"
                    ).dt.date
                ...
                with c1:
                    users_opt = ["(Todos)"] + sorted(
                        [u for u in df_log["user"].dropna().unique().tolist()]
                    )
                ...
                if "ts" in logv.columns:
                    logv["_d"] = pd.to_datetime(
                        logv["ts"].str.replace("Z", "", regex=False), errors="coerce"
                    ).dt.date
                    if dt_min:
...
# =============================================================================
# >>> DAQUI PRA BAIXO: parsing, gráficos, etc.
# =============================================================================
...
def _detecta_usina(linhas: List[str]) -> Optional[str]:
    for sline in linhas:
        if re.search(r"(?i)\busina:", sline):
            ...
            m = re.search(
                r"(?i)usina:\s*([A-Za-zÀ-ÿ0-9 .\-]+?)(?:\s+sa[ií]da\s+da\s+usina\b|$)", s0
            )
            if m:
        ...
        if len(partes) >= 5 and cp_regex.match(partes[0]):
            try:
                ...
                if i_data is not None:
                    i_tipo = next(
                        (i for i in range(i_data + 1, len(partes)) if tipo_token.match(partes[i])),
                        None,
                    )
...
def compute_exec_kpis(df_view: pd.DataFrame, fck_val: Optional[float]):
    ...
    pct63 = _pct_hit(63)
    media_geral = (
        float(pd.to_numeric(df_view["Resistência (MPa)"], errors="coerce").mean())
        if not df_view.empty
        else None
    )
    dp_geral = (
        float(pd.to_numeric(df_view["Resistência (MPa)"], errors="coerce").std())
        if not df_view.empty
        else None
    )
...
def render_print_block(pdf_all: bytes, pdf_cp: Optional[bytes], brand: str, brand600: str):
    ...
    if pdf_cp:
        ...
        cp_btn = (
            f'<button class="h-print-btn" onclick="habiPrint(\'{b64_cp}\')">🖨️ Imprimir — CP focado</button>'
        )
    html = f"""
    <style>
...
if BATCH_MODE:
    uploaded_files = st.file_uploader(
        "📁 PDF(s)", type=["pdf"], accept_multiple_files=True, key=_uploader_key
    )
else:
    up1 = st.file_uploader(
        "📁 PDF (1 arquivo)", type=["pdf"], accept_multiple_files=False, key=_uploader_key
    )
...
def render_overview_and_tables(df_view: pd.DataFrame, stats_cp_idade: pd.DataFrame, TOL_MP: float):
    ...
    def _to_date(d):
        try:
            return _dt.strptime(str(d), "%d/%m/%Y").date()
        except Exception:
            return None
    ...
    if not df_view.empty:
        ...
        if datas_validas:
            ...
            data_label = (
                di.strftime("%d/%m/%Y")
                if di == df_
                else f"{di.strftime('%d/%m/%Y')} — {df_.strftime('%d/%m/%Y')}"
            )
    ...
    with e3:
        ...
        st.markdown(
            f'<div class="h-card"><div class="h-kpi-label">Relatórios lidos</div><div class="h-kpi">{n_relatorios}</div></div>',
            unsafe_allow_html=True,
        )
    with e4:
        snf = _pd.to_numeric(df_view.get("Abatimento NF (mm)"), errors="coerce")
        stol = (
            _pd.to_numeric(df_view.get("Abatimento NF tol (mm)"), errors="coerce")
    ...
    def _hits(df_src, age, fck):
        if fck is None or _pd.isna(fck):
            return (0, 0)
...
if uploaded_files:
        ...
        if not df_i.empty:
            ...
            if "Fck Projeto" in df_i.columns:
                ...
                if scalar_fck is not None:
                    df_i["Fck Projeto"] = pd.to_numeric(df_i["Fck Projeto"], errors="coerce").fillna(
                        float(scalar_fck)
                    )
            else:
    ...
    if not frames:
        ...
        if not df.empty:
            ...
            nf_multi = (
                nf_rel.groupby(["Nota Fiscal"])["Relatório"].nunique().reset_index(name="n_rel")
            )
            ...
            cp_multi = (
                cp_rel.groupby(["CP"])["Relatório"].nunique().reset_index(name="n_rel")
            )
        ...
        if multiple_fck_detected:
            ...
            selected_fck_label = st.selectbox(
                "fck para análise", fck_labels, format_func=lambda lbl: lbl if lbl != "—" else "Não informado"
            )
        ...
        fck_active = (
            float(fck_series_focus.mode().iloc[0])
            if not fck_series_focus.empty
            else (float(fck_series_all_g.mode().iloc[0]) if not fck_series_all_g.empty else None)
        )
        ...
        if est_df is not None:
            ...
            if CAN_EXPORT:
                ...
                st.download_button(
                    "🖼️ Baixar Gráfico 3 (PNG)",
                    data=_buf3.getvalue(),
                    file_name="grafico3_comparacao.png",
                    mime="image/png",
                )
            ...
            cond_df = pd.DataFrame(
                {
                    ...
                    "Estimado (MPa)": est_df.set_index("Idade (dias)")["Resistência (MPa)"]
                    .reindex([7, 28, 63])
                    .values,
                }
            )
        ...
        # ===== Gráfico 4 — Pareamento ponto-a-ponto (SISTEMA + PDF, mas PDF sem tabela)
        ...
        if est_df is not None and not est_df.empty:
            ...
            # real e estimado LIGADOS (como no gráfico 1)
            for cp, sub in df_plot.groupby("CP"):
                sub = sub.sort_values("Idade (dias)")
            ...
            # aqui só mostramos no sistema
            st.write("#### 📑 Pareamento ponto-a-ponto (apenas no sistema)")
        ...
        def gerar_pdf(
            ...
            pareamento_df: Optional[pd.DataFrame],  # <== recebido, mas não vamos imprimir a tabela
            ...
            if verif_fck_df is not None and not verif_fck_df.empty:
                ...
                for _, r in verif_fck_df.iterrows():
                    rows_v.append(
                        [
                            ...
                            f"{r.get('fck Projeto (MPa)', float('nan')):.3f}"
                            if pd.notna(r.get("fck Projeto (MPa)", float("nan")))
                            else "—",
                            r.get("Status", "—"),
                        ]
                    )
                ...
                # colorir apenas a coluna de Status
                def _status_bg(text: str):
                    ...
                    if "atingiu" in t or "dentro" in t:
                ...
                for r_i, row in enumerate(rows_v[1:], start=1):
                    bg = _status_bg(row[3])
                    if bg:
                        extra_styles.append(("BACKGROUND", (3, r_i), (3, r_i), bg))
                        extra_styles.append(("TEXTCOLOR", (3, r_i), (3, r_i), _C.black))
                        extra_styles.append(("FONTNAME", (3, r_i), (3, r_i), "Helvetica-Bold"))
                if extra_styles:
                    tv.setStyle(TableStyle(extra_styles))
                story.append(tv)
                story.append(Spacer(1, 8))
            ...
            if cond_df is not None and not cond_df.empty:
                ...
                # colorir status
                extra_styles_c = []
                for r_i, row in enumerate(rows_c[1:], start=1):
                    bg = _status_bg(row[4])
                    if bg:
                        extra_styles_c.append(("BACKGROUND", (4, r_i), (4, r_i), bg))
                        extra_styles_c.append(("TEXTCOLOR", (4, r_i), (4, r_i), _C.black))
                        extra_styles_c.append(("FONTNAME", (4, r_i), (4, r_i), "Helvetica-Bold"))
                if extra_styles_c:
                    tc.setStyle(TableStyle(extra_styles_c))
                story.append(tc)
                story.append(Spacer(1, 8))

            # >>> ATENÇÃO: aqui NÃO vamos incluir a tabela de pareamento ponto a ponto
            # (ficou só no sistema), mas o gráfico 4 já foi incluído acima.
            ...
            if pv_cp_status is not None and not pv_cp_status.empty:
                ...
                for r_i, row in enumerate(data_rows, start=1):
                    for c_i in [i_s7, i_s28, i_s63]:
                        if c_i >= 0:
                            bg = _status_bg(row[c_i])
                            if bg:
                                extra_det.append(("BACKGROUND", (c_i, r_i), (c_i, r_i), bg))
                                extra_det.append(("TEXTCOLOR", (c_i, r_i), (c_i, r_i), _C.black))
                                extra_det.append(("FONTNAME", (c_i, r_i), (c_i, r_i), "Helvetica-Bold"))
                    if i_alert >= 0:
                        bg2 = _alerta_bg(row[i_alert])
                        if bg2:
                            extra_det.append(("BACKGROUND", (i_alert, r_i), (i_alert, r_i), bg2))
                            extra_det.append(("TEXTCOLOR", (i_alert, r_i), (i_alert, r_i), _C.black))
                            extra_det.append(("FONTNAME", (i_alert, r_i), (i_alert, r_i), "Helvetica-Bold"))
                if extra_det:
                    t_det.setStyle(TableStyle(extra_det))
                story.append(t_det)
        ...
        if has_df and CAN_EXPORT:
            try:
                pdf_bytes = gerar_pdf(
                    ...
                    (
                        (lambda _d: ((min(_d).strftime("%d/%m/%Y") if min(_d) == max(_d) else f"{min(_d).strftime('%d/%m/%Y')} — {max(_d).strftime('%d/%m/%Y')}")) if _d else "—")(
                            [d for d in df["_DataObj"].dropna().tolist()]
                            if "_DataObj" in df.columns
                            else []
                        )
                    ),
                ...
                log_event(
                    "export_pdf",
                    {
                        ...
                        "obra": str(df_view["Obra"].mode().iat[0])
                        if "Obra" in df_view.columns and not df_view["Obra"].dropna().empty
                        else "—",
                        "file_name": file_name_pdf,
                    },
