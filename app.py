...
def _img_from_fig(_fig, w=620, h=420):
...
if uploaded_files:
    ...
    if not frames:
        ...
        # ==============================
        # 🔴 Verificação crítica: NF/CP repetidos entre relatórios diferentes
        # ==============================
        def _normstr(x):
            if pd.isna(x):
                return None
        ...
        df_view["Relatório"] = df_view["Relatório"].astype(str)
        df_view["Nota Fiscal (norm)"] = df_view["Nota Fiscal"].apply(_normstr)
        df_view["CP (norm)"] = df_view["CP"].apply(_normstr)

        # NF repetida em relatórios distintos
        nf_rel_counts = df_view.dropna(subset=["Nota Fiscal (norm)"]).groupby("Nota Fiscal (norm)")["Relatório"].nunique()
        nfs_problema = nf_rel_counts[nf_rel_counts > 1].index.tolist()

        # CP repetido em relatórios distintos
        cp_rel_counts = df_view.dropna(subset=["CP (norm)"]).groupby("CP (norm)")["Relatório"].nunique()
        cps_problema = cp_rel_counts[cp_rel_counts > 1].index.tolist()

        if nfs_problema:
            st.error("🚨 **NF repetida em relatórios diferentes** — isso é gravíssimo para emissão de certificado.")
            bad_nf_rows = df_view[df_view["Nota Fiscal (norm)"].isin(nfs_problema)][
                ["Relatório", "Nota Fiscal", "CP", "Idade (dias)", "Resistência (MPa)", "Arquivo"]
            ].sort_values(["Nota Fiscal", "Relatório", "CP", "Idade (dias)"])
            st.dataframe(bad_nf_rows, use_container_width=True)
        if cps_problema:
            st.error("🚨 **CP repetido em relatórios diferentes** — verifique a origem dos CPs.")
            bad_cp_rows = df_view[df_view["CP (norm)"].isin(cps_problema)][
                ["Relatório", "CP", "Nota Fiscal", "Idade (dias)", "Resistência (MPa)", "Arquivo"]
            ].sort_values(["CP", "Relatório", "Idade (dias)"])
            st.dataframe(bad_cp_rows, use_container_width=True)
        if nfs_problema or cps_problema:
            st.warning("⚠️ Corrija as inconsistências antes de emitir certificados. Os dados abaixo seguem para análise, mas estão marcados com alertas.")
        ...
        df_view = df_view.drop(columns=["_FckLabel", "Nota Fiscal (norm)", "CP (norm)"], errors="ignore")
        ...
        with k1:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Obra</div><div class="h-kpi">{obra_label}</div></div>',
                unsafe_allow_html=True,
            )
        with k2:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Data da moldagem</div><div class="h-kpi">{data_label}</div></div>',
                unsafe_allow_html=True,
            )
        with k3:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">fck de projeto (MPa)</div><div class="h-kpi">{fck_label}</div></div>',
                unsafe_allow_html=True,
            )
        with k4:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Tolerância aplicada (MPa)</div><div class="h-kpi">±{TOL_MP:.1f}</div></div>',
                unsafe_allow_html=True,
            )
        with k5:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">CPs com fck 28d</div><div class="h-kpi">{fmt_pct(KPIs["pct28"])}</div></div>',
                unsafe_allow_html=True,
            )
        with k6:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">CPs com fck 63d</div><div class="h-kpi">{fmt_pct(KPIs["pct63"])}</div></div>',
                unsafe_allow_html=True,
            )
        ...
        with e1:
            ...
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Média geral</div><div class="h-kpi">{media_txt}</div></div>',
                unsafe_allow_html=True,
            )
        with e2:
            dp_txt = "--" if KPIs["dp"] is None else f"{KPIs['dp']:.1f}"
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Desvio-padrão</div><div class="h-kpi">{dp_txt}</div></div>',
                unsafe_allow_html=True,
            )
        with e3:
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Relatórios</div><div class="h-kpi">{KPIs["n_rel"]}</div></div>',
                unsafe_allow_html=True,
            )
        with e4:
            snf = pd.to_numeric(df_view.get("Abatimento NF (mm)"), errors="coerce")
            stol = (
                pd.to_numeric(df_view.get("Abatimento NF tol (mm)"), errors="coerce")
                if "Abatimento NF tol (mm)" in df_view.columns
            ...
            st.markdown(
                f'<div class="h-card"><div class="h-kpi-label">Abatimento NF</div><div class="h-kpi">{abat_nf_label}</div></div>',
                unsafe_allow_html=True,
            )

        if multiple_fck_detected:
            st.markdown(
                "<div class='pill' style='background:rgba(250,204,21,.16); color:#b45309; font-weight:700;'>⚠️ FCK Análise = Atenção — Certificado com 02 Fck</div>",
                unsafe_allow_html=True,
            )
        ...
        if (p28 is not None) or (p63 is not None):
            score = (0 if p28 is None else 0.6 * p28) + (0 if p63 is not None else 0.4 * p63)
        ...
        linhas = []
        m7_v = verif_fck_df.loc[verif_fck_df["Idade (dias)"] == 7, "Média Real (MPa)"].iloc[0] if not verif_fck_df.empty else float("nan")
        if pd.notna(m7_v):
            linhas.append(_badge(f"7 dias • média {m7_v:.2f} MPa", color="#f59e0b"))
        else:
        ...
        if fck_active is None:
            ...
            m28_v = verif_fck_df.loc[verif_fck_df["Idade (dias)"] == 28, "Média Real (MPa)"].iloc[0] if not verif_fck_df.empty else float("nan")
            m63_v = verif_fck_df.loc[verif_fck_df["Idade (dias)"] == 63, "Média Real (MPa)"].iloc[0] if not verif_fck_df.empty else float("nan")
            if pd.isna(m28_v):
                linhas.append(_badge("28 dias • sem dados", color="#9ca3af"))
            else:
                ok28 = m28_v >= fck_active
                linhas.append(
                    _badge(
                        f"28 dias • {'atingiu' if ok28 else 'não atingiu'} fck ({m28_v:.2f} {'≥' if ok28 else '<'} {fck_active:.2f} MPa)",
            ...
            if pd.isna(m63_v):
                ...
                ok63 = m63_v >= fck_active
                linhas.append(
                    _badge(
                        f"63 dias • {'atingiu' if ok63 else 'não atingiu'} fck ({m63_v:.2f} {'≥' if ok63 else '<'} {fck_active:.2f} MPa)",
        ...
        # ===== Verificação detalhada por CP (7/28/63) com suporte a pares 7/7, 28/28, 63/63 e alerta de divergência > 2 MPa =====
        st.markdown("#### ✅ Verificação detalhada por CP (7/28/63 dias, pares 7/7 • 28/28 • 63/63)")
        pv_cp_status = None
        ...
        if tmp_v.empty:
            ...
            pv_multi = (
                tmp_v.pivot_table(index="CP", columns=["Idade (dias)", "rep"], values="MPa", aggfunc="first").sort_index(axis=1)
            )

            # Garante ao menos uma coluna por idade (mesmo que NaN)
            for age in [7, 28, 63]:
                if age not in pv_multi.columns.get_level_values(0):
                    pv_multi[(age, 1)] = pd.NA

            # Ordena por (7,1..n) (28,1..n) (63,1..n)
            ordered = []
            for age in [7, 28, 63]:
            ...
            # 28d: todos os valores (todas as réplicas) precisam estar >= fck
            ...
            # Alerta de divergência entre pares > 2 MPa para 7/7, 28/28, 63/63
            pair_alert_msgs = []
            ORANGE = "🟠"
            TPAIR = 2.0

            def _pair_check(values: List[float], label: str) -> Optional[str]:
                vals = [v for v in values if pd.notna(v)]
                if len(vals) >= 2:
                    if max(vals) - min(vals) > TPAIR:
                        return f"{label}: diferença entre pares > {TPAIR:.1f} MPa"
                return None

            # monta lista de alertas por linha (CP)
            for idx in pv_multi.index:
                alerts = []
                # 7d pares
                if 7 in pv_multi.columns.get_level_values(0):
                    vals7 = pv_multi.loc[idx, 7].tolist()
                    msg = _pair_check(vals7, "7d")
                    if msg:
                        alerts.append(msg)
                # 28d pares
                if 28 in pv_multi.columns.get_level_values(0):
                    vals28 = pv_multi.loc[idx, 28].tolist()
                    msg = _pair_check(vals28, "28d")
                    if msg:
                        alerts.append(msg)
                # 63d pares
                if 63 in pv_multi.columns.get_level_values(0):
                    vals63 = pv_multi.loc[idx, 63].tolist()
                    msg = _pair_check(vals63, "63d")
                    if msg:
                        alerts.append(msg)
                pair_alert_msgs.append("; ".join(alerts) if alerts else "")

            # rótulos de status por idade
            status_df = pd.DataFrame(
                {
                    "Status 7d": [_status_text_media(v, 7, fck_active2) for v in media_7.reindex(pv_multi.index)],
                    "Status 28d": [("🟢 Atingiu fck" if v else ("🔴 Não atingiu fck" if v is False else "⚪ Sem dados")) for v in ok28.reindex(pv_multi.index)],
                    "Status 63d": [_status_text_media(v, 63, fck_active2) for v in media_63.reindex(pv_multi.index)],
                    "Alertas": pair_alert_msgs,
                },
            ...
            # Prefixa CP com 🟠 quando há alerta de par > 2 MPa
            pv.loc[pv["Alertas"].astype(str).str.len() > 0, "CP"] = ORANGE + " " + pv.loc[
                pv["Alertas"].astype(str).str.len() > 0, "CP"
            ].astype(str)
            ...
            ordered_cols = (
                ...
                + (["Alertas"] if "Alertas" in pv.columns else [])
            )
            ...
            ordered_cols = [
                "7 dias — Status" if c == "Status 7d" else "28 dias — Status" if c == "Status 28d" else "63 dias — Status" if c == "Status 63d" else c
        ...
        # =============================================================================
        # PDF — Cabeçalho completo + gráficos maiores + detalhamento CP
        # =============================================================================
        ...
        def gerar_pdf(
            ...
            doc = SimpleDocTemplate(
                buffer, pagesize=pagesize, leftMargin=18, rightMargin=18, topMargin=26, bottomMargin=56
            )
            ...
            if fig1:
                story.append(_img_from_fig_pdf(fig1, w=640, h=430))
                story.append(Spacer(1, 8))
            if fig2:
                story.append(_img_from_fig_pdf(fig2, w=600, h=400))
                story.append(Spacer(1, 8))
            if fig3:
                story.append(_img_from_fig_pdf(fig3, w=640, h=430))
                story.append(Spacer(1, 8))
            if fig4:
                story.append(_img_from_fig_pdf(fig4, w=660, h=440))
                story.append(Spacer(1, 8))
            ...
            # ===== Verificação detalhada por CP (inclui coluna Alertas)
        ...
        if has_df:
            try:
                pdf_bytes = gerar_pdf(
                    ...
                    data_label,
                    _format_float_label_local(fck_active),
                    verif_fck_df,
                    cond_df,
                    pareamento_df,
                    pv_cp_status,
            ...
            try:
                stats_all_full = (
                    df_view.groupby("Idade (dias)")["Resistência (MPa)"].agg(mean="mean", std="std", count="count").reset_index()
                )
