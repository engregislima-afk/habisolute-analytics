...
# =============================================================================
# Validação / Consistência (NF e CP entre Relatórios) — NOVO
# =============================================================================
def _norm_id_token(x: Any) -> Optional[str]:
    """Normaliza um código (NF/CP): remove não-dígitos, tira zeros à esquerda. Retorna None para vazios."""
    if x is None:
        return None
    t = re.sub(r"\D+", "", str(x))
    t = t.lstrip("0")
...
def validar_conflitos_nf_cp(df_src: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    Retorna dicionário com:
      - nf_conflitos: mesma NF_norm em mais de 1 Relatório
      - cp_conflitos: mesmo CP_norm em mais de 1 Relatório
    """
...
if uploaded_files:
    ...
    if not frames:
        ...
        # ===== Validação crítica: NF/CP não podem cruzar relatórios (NOVO)
        ...
        linhas = []
        m7_row = verif_fck_df.loc[verif_fck_df["Idade (dias)"]==7,"Média Real (MPa)"].iloc[0] if not verif_fck_df.empty else float("nan")
        if pd.notna(m7_row): linhas.append(_badge(f"7 dias • média {m7_row:.2f} MPa", color="#f59e0b"))
        ...
        if fck_active is None:
            ...
            m28_row = verif_fck_df.loc[verif_fck_df["Idade (dias)"]==28,"Média Real (MPa)"].iloc[0] if not verif_fck_df.empty else float("nan")
            m63_row = verif_fck_df.loc[verif_fck_df["Idade (dias)"]==63,"Média Real (MPa)"].iloc[0] if not verif_fck_df.empty else float("nan")
            if pd.isna(m28_row):
                linhas.append(_badge("28 dias • sem dados", color="#9ca3af"))
            else:
                ok28 = m28_row >= fck_active
                linhas.append(_badge(
                    f"28 dias • {'atingiu' if ok28 else 'não atingiu'} fck "
                    f"({m28_row:.2f} {'≥' if ok28 else '<'} {fck_active:.2f} MPa)",
                    color=("#16a34a" if ok28 else "#ef4444")
                ))
            if pd.isna(m63_row):
                linhas.append(_badge("63 dias • sem dados", color="#9ca3af"))
            else:
                ok63 = m63_row >= fck_active
                linhas.append(_badge(
                    f"63 dias • {'atingiu' if ok63 else 'não atingiu'} fck "
                    f"({m63_row:.2f} {'≥' if ok63 else '<'} {fck_active:.2f} MPa)",
        ...
        # ===== Verificação detalhada por CP (7/28/63) — guarda DataFrame para PDF
        ...
        if has_df:
            if conflicts_flag:
                st.error("⛔ Exportações bloqueadas: existem conflitos de NF/CP entre relatórios. "
                         "Ajuste os dados (ou corrija a leitura) e recarregue para emitir o relatório.")
            else:
                try:
                    pdf_bytes = gerar_pdf(
