# app.py — Habisolute Analytics (com histórico, OCR de fallback e alertas configuráveis)
...
HISTORY_PARQUET = PREFS_DIR / "history.parquet"
HISTORY_CSV = PREFS_DIR / "history.csv"
...
s.setdefault("alert_min_28", 80.0)
s.setdefault("alert_min_63", 80.0)
...
with st.sidebar:
    ...
    page_sel = st.radio("Página", ["Análise atual", "Histórico / Tendências"], index=0)
    ...
    with st.expander("🔔 Alertas dinâmicos"):
        s["alert_min_28"] = st.slider("Mínimo de CPs ≥ fck aos 28d (%)", 0, 100, int(s["alert_min_28"]), 1)
        s["alert_min_63"] = st.slider("Mínimo de CPs ≥ fck aos 63d (%)", 0, 100, int(s["alert_min_63"]), 1)
        st.caption("Se os indicadores ficarem abaixo disso, mostramos alerta na análise.")
    st.markdown("---")
...
# ===== OCR de fallback (suave) =====
def _try_ocr_pdf(raw: bytes) -> List[str]:
    """Tenta extrair texto via OCR se não achou nada com pdfplumber. Só roda se libs existirem."""
    linhas_ocr: List[str] = []
    try:
        from pdf2image import convert_from_bytes
        import pytesseract
    except Exception:
        # libs não estão instaladas — só avisa
        return []
    try:
...
def extrair_dados_certificado(uploaded_file):
    # agora retorna também debug: linhas não lidas + linhas brutas
    try:
    ...
    # Se pdfplumber não achou quase nada, tenta OCR
    if not pdf_text_found or len(linhas_todas) < 6:
        ocr_lines = _try_ocr_pdf(raw)
        if ocr_lines:
            linhas_todas = ocr_lines
        ...
        if len(partes) >= 5 and cp_regex.match(partes[0]):
            try:
                ...
                if idade is None or resistência is None:
                    # não conseguiu montar CP — salva como unparsed
                    unparsed_lines.append(sline)
                    continue
                ...
                parsed_ok = False
        if not parsed_ok:
            if sline and not sline.startswith("Relatório:"):
                unparsed_lines.append(sline)
    ...
    debug_info = {
        "raw_lines": linhas_todas,
        "unparsed_lines": list(dict.fromkeys(unparsed_lines))
    }

    return df, obra, data_relatorio, fck_projeto, debug_info

# =============================================================================
# Persistência de histórico
# =============================================================================
def append_history(df: pd.DataFrame):
    """Salva no histórico todos os registros lidos agora."""
    if df is None or df.empty:
        return
    hist_df = None
    df = df.copy()
    df["parsed_at"] = datetime.utcnow().isoformat(timespec="seconds")
    df["parsed_by"] = s.get("username") or "anon"

    # tenta parquet primeiro
    try:
        if HISTORY_PARQUET.exists():
            hist_df = pd.read_parquet(HISTORY_PARQUET)
        if hist_df is not None and not hist_df.empty:
            all_df = pd.concat([hist_df, df], ignore_index=True)
        else:
            all_df = df
        all_df.to_parquet(HISTORY_PARQUET, index=False)
        return
    except Exception:
        pass
    ...
    try:
        if HISTORY_CSV.exists():
            hist_df = pd.read_csv(HISTORY_CSV, sep=";", encoding="utf-8")
            all_df = pd.concat([hist_df, df], ignore_index=True)
        else:
            all_df = df
        all_df.to_csv(HISTORY_CSV, sep=";", index=False, encoding="utf-8")
    except Exception:
        pass
...
def load_history() -> pd.DataFrame:
    ...
    if HISTORY_CSV.exists():
        try:
            return pd.read_csv(HISTORY_CSV, sep=";", encoding="utf-8")
        except Exception:
            pass
    return pd.DataFrame()
...
def render_historico_page():
    st.subheader("📦 Histórico de Certificados Lidos")
    hist = load_history()
    if hist.empty:
        st.info("Ainda não há histórico salvo. Faça leituras na página 'Análise atual'.")
        return
    # filtros
    c1, c2, c3 = st.columns([1.2, 1.2, 1.2])
    with c1:
        obras = ["(Todas)"] + sorted(hist["Obra"].dropna().astype(str).unique().tolist()) if "Obra" in hist.columns else ["(Todas)"]
        f_obra = st.selectbox("Obra", obras, index=0)
    with c2:
        usinas = ["(Todas)"] + sorted(hist["Usina"].dropna().astype(str).unique().tolist()) if "Usina" in hist.columns else ["(Todas)"]
        f_usina = st.selectbox("Usina", usinas, index=0)
    with c3:
        usuarios = ["(Todos)"] + sorted(hist["parsed_by"].dropna().astype(str).unique().tolist()) if "parsed_by" in hist.columns else ["(Todos)"]
        f_user = st.selectbox("Lido por", usuarios, index=0)

    v = hist.copy()
    if f_obra != "(Todas)":
        v = v[v["Obra"].astype(str) == f_obra]
    if f_usina != "(Todas)":
        v = v[v["Usina"].astype(str) == f_usina]
    if f_user != "(Todos)":
    ...
    with k1:
        st.markdown(f'<div class="h-card"><div class="h-kpi-label">CPs total</div><div class="h-kpi">{len(v)}</div></div>', unsafe_allow_html=True)
    with k2:
        st.markdown(f'<div class="h-card"><div class="h-kpi-label">Relatórios distintos</div><div class="h-kpi">{v["Relatório"].nunique() if "Relatório" in v.columns else 0}</div></div>', unsafe_allow_html=True)
    with k3:
        st.markdown(f'<div class="h-card"><div class="h-kpi-label">Obras distintas</div><div class="h-kpi">{v["Obra"].nunique() if "Obra" in v.columns else 0}</div></div>', unsafe_allow_html=True)
    with k4:
        st.markdown(f'<div class="h-card"><div class="h-kpi-label">Usuários</div><div class="h-kpi">{v["parsed_by"].nunique() if "parsed_by" in v.columns else 0}</div></div>', unsafe_allow_html=True)

    # gráfico de tendência: % aos 28d por mês
    if "parsed_at" in v.columns and "Idade (dias)" in v.columns and "Resistência (MPa)" in v.columns and "Fck Projeto" in v.columns:
        tmp = v.copy()
        tmp["parsed_at"] = pd.to_datetime(tmp["parsed_at"], errors="coerce")
        tmp = tmp.dropna(subset=["parsed_at"])
        tmp["ym"] = tmp["parsed_at"].dt.to_period("M").dt.to_timestamp()
        tmp28 = tmp[tmp["Idade (dias)"] == 28].copy()
        tmp28["Fck Projeto"] = pd.to_numeric(tmp28["Fck Projeto"], errors="coerce")
        tmp28["Resistência (MPa)"] = pd.to_numeric(tmp28["Resistência (MPa)"], errors="coerce")
        tmp28 = tmp28.dropna(subset=["Fck Projeto", "Resistência (MPa)"])
        if not tmp28.empty:
            grouped = tmp28.groupby("ym").apply(lambda g: (g["Resistência (MPa)"] >= g["Fck Projeto"]).mean() * 100.0).reset_index(name="pct28")
            fig, ax = plt.subplots(figsize=(7.5, 3.6))
            ax.plot(grouped["ym"], grouped["pct28"], marker="o")
            ax.set_title("Tendência histórica — CPs ≥ fck aos 28d")
            ax.set_ylabel("% de CPs")
            ax.grid(True, linestyle="--", alpha=0.4)
            st.pyplot(fig)
    st.write("#### Tabela histórica")
    st.dataframe(v, use_container_width=True)
    st.download_button("⬇️ Baixar histórico (CSV)", data=v.to_csv(index=False).encode("utf-8"),
                       file_name="historico_habisolute.csv", mime="text/csv")
...
def render_overview_and_tables(df_view: pd.DataFrame, stats_cp_idade: pd.DataFrame, TOL_MP: float):
    ...
    # alerta com base nas configs
    if KPIs["pct28"] is not None and KPIs["pct28"] < s.get("alert_min_28", 80):
        st.warning(f"⚠️ Percentual aos 28d ({KPIs['pct28']:.0f}%) ficou abaixo do limite configurado ({s.get('alert_min_28')}%).")
    if KPIs["pct63"] is not None and KPIs["pct63"] < s.get("alert_min_63", 80):
        st.warning(f"⚠️ Percentual aos 63d ({KPIs['pct63']:.0f}%) ficou abaixo do limite configurado ({s.get('alert_min_63')}%).")
...
# =============================================================================
# Uploader (somente se página for análise)
# =============================================================================
if page_sel == "Histórico / Tendências":
    ...
    if uploaded_files:
        ...
        for idx, f in enumerate(uploaded_files, start=1):
            ...
            df_i, obra_i, data_i, fck_i, debug_i = extrair_dados_certificado(f)
            if not df_i.empty:
                ...
                frames.append(df_i)
                append_history(df_i)
                st.success(f"✅ Certificado '{getattr(f, 'name', 'arquivo.pdf')}' lido com sucesso ({len(df_i)} linha(s)).")
                log_event("file_parsed", {
                    ...
                    "obra": obra_i,
                    "data_cert": data_i,
                    "unparsed_lines": len(debug_i.get("unparsed_lines", []))
                })
            if debug_i.get("unparsed_lines"):
                unparsed_all.extend([f"[{getattr(f,'name','arquivo.pdf')}] {ln}" for ln in debug_i["unparsed_lines"]])
        ...
        if not frames:
            ...
            # ===== mostra linhas não reconhecidas =====
            if unparsed_all:
                with st.expander("⚠️ Linhas não reconhecidas (para ajuste de layout)", expanded=False):
                    st.write("Essas linhas estavam no PDF mas não viraram CP. Você pode usar isso para melhorar o parser.")
                    for ln in unparsed_all[:300]:
                        st.code(ln)
            ...
            with st.expander("1) 📦 Dados lidos / visão geral", expanded=True):
                ...
                # idades fora do set principal
                principal = {3, 7, 14, 28, 63}
                extra_idades = sorted(set(df_view["Idade (dias)"]) - principal)
                if extra_idades:
                    st.info(f"Idades adicionais encontradas nos certificados: {', '.join(str(x) for x in extra_idades)}")
